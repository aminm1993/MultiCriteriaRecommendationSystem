...
public class Service2Activity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState)  {
       ...
       /* gv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        ...
        if (extras != null) {
           Consts.index = extras.getInt("index");
            ...
            if(Consts.index==-1)
                ...
                serviceModel=ServicesActivity.models.get(Consts.index);
            ...
            if(serviceModel.Details==null || serviceModel.Details.size()==0) {
                ...
                Consts.temp= Sort(serviceModel.Details);
                Consts.adapter = new CategoryAdapter(a, Sort(serviceModel.Details), Consts.index, 0);
                categoryID = serviceModel.ID;
                if (serviceModel.Name.contains("کتاب"))
                    orderNotExist.setVisibility(View.VISIBLE);


                gv.setAdapter(Consts.adapter);
                       ...
                       /* if(position==0)
                        {
                            ...
                            {
                            for (int i = 0; i < serviceModel.Details.size(); i++) {
                                if (serviceModel.Details.get(i).CategoryId.equals(Consts.spinnerIds[position])) {
                                    search.add(serviceModel.Details.get(i));
                                }
                            }
                        }
