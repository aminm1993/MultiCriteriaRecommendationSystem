...
/**
 * This is a Bidirectional implementation for Time Parametrized Shortest Path algorithm.
 * @author Amin Mohammadpour
 */
public class TP_SP_Bidirectional_Advance_4 {
    /**
    ...
    public static boolean find_optimal_route(final Graph G, final Time_Slots_of_Day time_slots_of_day, final int s,
        ...
        final ConcurrentMap<Integer, CostVector[]> M = new ConcurrentHashMap<Integer, CostVector[]>(8, 0.9f, 1);
        ...
        Thread backward_thread = new Thread() {
            public void run() {
//                final LinkedBlockingQueue<Integer> Q = new LinkedBlockingQueue<Integer>(1);
                ...
                M.put(d, new CostVector[time_slots_of_day.get_slots_number()]);
                for (Integer time_slot_index : time_slots_of_day.getTime_slots().keySet()) {
                    M.get(d)[time_slot_index] = cost_vector_d;
                }
/*                try {
                    Q.put(d);
...
//                while (!Q.isEmpty() && !forward_thread_status.isCompleted()) {
                while (!forward_thread_status.isCompleted()) {
/*                    try {
                        next = Q.take();
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }*/
                    final Integer settled_node_id = Q.poll();
                    if(settled_node_id != null) {
                        executor_service.execute(new Runnable() {
                            @Override
                            public void run() {
//                            int settled_node_id = Q.poll();
                                CostVector cost_vector;
                                int attribute_index, next_time_slot_index;
                                LocalTime travel_time;
                                float[] costs;
                                //iterate on the in-going edges of the current node to continue branching to its predecessors
                                for (Integer in_edge_index : G.get_node(settled_node_id).getIn_edges()) {
                                    Edge in_edge = G.get_edge(in_edge_index);
                                    int predecessor_node_id = in_edge.getNode_1_id();
                                    CostVector[] cost_vectors_for_settled_node = new CostVector[time_slots_of_day.get_slots_number()];
                                    ...
                                    for (Integer time_slot_index : time_slots_of_day.getTime_slots().keySet()) {
                                        ...
                                        if (M.get(settled_node_id)[next_time_slot_index] != null) {
                                            for (String attribute_name : selected_attributes) {
                                                costs[attribute_index] = in_edge.getAttributes().get(attribute_name).get_weight(time_slot_index)
                                                        + M.get(settled_node_id)[next_time_slot_index].getCost(attribute_index);
                                                attribute_index++;
                                            }
                                            ...
                                            if (!U.is_dominant_on(cost_vector)) {
                                                if (!M.containsKey(predecessor_node_id)) {
                                                    cost_vectors_for_settled_node[time_slot_index] = cost_vector;
                                                } else {
                                                    if (M.get(predecessor_node_id)[time_slot_index] != null) {
                                                        if (cost_vector.is_dominant_on(M.get(predecessor_node_id)[time_slot_index])) {
                                                            M.get(predecessor_node_id)[time_slot_index] = cost_vector; //overwrite the previous cost vector
                                                        }
                                                    } else {
                                                        M.get(predecessor_node_id)[time_slot_index] = cost_vector;
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if (M.get(predecessor_node_id).length > 0) {
                                        Q.offer(predecessor_node_id);
/*                                    try {
                                        Q.put(predecessor_node_id);
                                    } catch (InterruptedException e) {
                                        e.printStackTrace();
                                    }*/
                                    }
            ...
            for (final CostVector non_dominated_cv : PQ) {
                ...
                for (Integer out_edge_index : G.get_node(non_dominated_cv.getNode_id()).getOut_edges()) {
                   ...
                   /*final Map<Integer, Double> overlapping_distribution = time_slots_of_day.
                    ...
                    settled_cost_vector = null;
                    try {
                        settled_cost_vector = M.get(successor_node_id)[time_slot_index];
                    }catch (NullPointerException ne){}
                    if (settled_cost_vector != null) {
