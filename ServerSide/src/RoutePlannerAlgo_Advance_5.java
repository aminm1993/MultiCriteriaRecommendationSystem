...
/**
 * This is the advanced Bidirectional implementation for Time Parametrized Shortest Path algorithm.
 * @author Amin Mohammadpour
 */
public class TP_SP_Bidirectional_Advance_5 {
    /**
    ...
    public static boolean find_optimal_route(final Graph G, final Time_Slots_of_Day time_slots_of_day, final int s,
        ...
        double factorized_distance = distance * 0.4;
        final Bound bound = new Bound(move(new GPSPoint(min_lat, min_lng,null), factorized_distance, 225),
        ...
        final ConcurrentMap<Integer, Map<Integer, CostVector>> M = new ConcurrentHashMap<Integer, Map<Integer, CostVector>>(8, 0.9f, 1);
        ...
        Thread backward_thread = new Thread() {
            public void run() {
                ...
                M.put(d, new HashMap<Integer, CostVector>());
                ...
                while (!Q.isEmpty() && !forward_thread_status.isCompleted()) {
                    int settled_node_id = Q.remove(0);
                    ...
                    for (Integer in_edge_index : G.get_node(settled_node_id).getIn_edges()) {
                        ...
                        node = G.get_node(predecessor_node_id);
                        if (bound.contains(node.getLat(), node.getLng())){
                                ...
                                if (M.get(settled_node_id).containsKey(next_time_slot_index)) {
                                    ...
                                    if (!U.is_dominant_on(cost_vector)) {
                                        if (!M.containsKey(predecessor_node_id)) {
                                            M.put(predecessor_node_id, new HashMap<Integer, CostVector>());
                                            M.get(predecessor_node_id).put(time_slot_index, cost_vector);
                                        } else {
                                            if (M.get(predecessor_node_id).containsKey(time_slot_index)) {
                                                if (cost_vector.is_dominant_on(M.get(predecessor_node_id).get(time_slot_index))) {
                            ...
                            if (M.containsKey(predecessor_node_id) && M.get(predecessor_node_id).size() > 0) {
                                Q.add(predecessor_node_id);
                            }
                        }
                    }
                }
                ...
                for (Integer out_edge_index : G.get_node(non_dominated_cv.getNode_id()).getOut_edges()) {
                   ...
                   /*final Map<Integer, Double> overlapping_distribution = time_slots_of_day.
                    ...
                    if (!backward_thread_status.isCompleted() || (backward_thread_status.isCompleted() && M.containsKey(successor_node_id))) {
                        /* First, check if the newly generated cost vector belongs to the destination node d then validate
                           it by verifying its dominance relationship with the cost vectors at the solution set. Otherwise,
                           verify its dominance relationship against the non-dominated cost vectors at recorded at the same
                           node it belongs to.*/
