...
RandIt read_risk_file(RandIt first,
                                      RandIt last,
                                      const Partition &partition,
                                      const Parameters &parameters)
{
	char [] address ="/accident.csv";
	...
	for (const auto &edge : shortest_path.path)
	{
		...
		double LAT = get_cell.getitem("LATITUDE");
		double LNG = get_cell.getitem("LONGITUDE");
		counter++;
	};
...
template <typename RandIt> RandIt filterViaCandidatesByUniqueNodeIds(RandIt first, RandIt last)
{
    ...
    read_risk_file(RandIt, std::random_access_iterator_tag);
                                               ...
                                               unsigned number_of_alternatives)
{
	...
	r_tree_sort(&it); // this sort is mine
