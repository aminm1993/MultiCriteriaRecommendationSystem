...
public class Best_Start_Time {
    ...
    public static boolean find_best_start_time(final Graph G, final Time_Slots_of_Day time_slots_of_day, final int s,
        ...
        Map<Integer, Map<Integer, CostVector>> M = new HashMap<Integer, Map<Integer, CostVector>>();
        ...
        M.put(s, new HashMap<Integer, CostVector>());
        for (Integer time_slot_index : time_slots_of_day.getTime_slots().keySet()) {
            ...
            M.get(s).put(time_slot_index, cost_vector);
        }
        PQ.add(s);
        int settled_node_id, successor_node_id, attribute_index;
        float aggregated_cost, travel_time;
        CostVector settled_cost_vector = null;
        Edge out_edge;
        Iterator<CostVector> iterator;
        Node node;
        boolean dominated_by_solution_set, locally_dominated;
        while (!PQ.isEmpty()) {
            ...
            for (Integer out_edge_index : G.get_node(settled_node_id).getOut_edges()) {
                ...
                if (bound.contains(node.getLat(), node.getLng())){
                    ...
                    for (Integer time_slot_index : time_slots_of_day.getTime_slots().keySet()) {
                        try {
                            settled_cost_vector = M.get(settled_node_id).get(time_slot_index);
                        } catch (Exception ex) {
                        }
                        if (settled_cost_vector != null) {
                            locally_dominated = false;
                                ...
                                if (!dominated_by_solution_set) {
                                    if (M.containsKey(successor_node_id)) {
                                        if (M.get(successor_node_id).containsKey(time_slot_index)) {
                                            if (!M.get(successor_node_id).get(time_slot_index).is_dominated_by(cost_vector)) {
                                                locally_dominated = true;
                                            }
                                        } else {
                                            M.get(successor_node_id).put(time_slot_index, cost_vector);
                                        }
                                    } else {
                                        M.put(successor_node_id, new HashMap<Integer, CostVector>());
                                        M.get(successor_node_id).put(time_slot_index, cost_vector);
                                    }
                                    if (!PQ.contains(successor_node_id) && !locally_dominated) {
